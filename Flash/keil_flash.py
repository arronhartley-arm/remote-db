...
from flashprogrammer.execution import ensureDeviceStopped, funcCall, funcCallRepeat, getWorkingRAMRequired, writeRegs, isV8M
...
class KeilFlash(FlashMethodv1):
    ...
    def setup(self):
        ...
        #   then stack
        #   increase stack size if remaining write buffer size means pages per write will comfortably hit MAX_REPEATS
        self.stackSize = 0x100
        writeBufferSize = self.ramAddr - (nextAddr + self.stackSize) + self.ramSize
        if (writeBufferSize / 2) / self.pageSize  > MAX_REPEATS:
            self.stackSize = 0x200

        self.stackBottom = nextAddr
        self.stackTop = self.stackBottom + self.stackSize - 4
        nextAddr += self.stackSize

        #   set MSPLIM to the bottom of the stack on v8M targets
        if (isV8M(self.dev)):
    ...
    def erase(self, addr, size):
        ...
        # Erasing a subset of the chip
            ...
            progress = self.operationStarted('Erasing %d bytes from 0x%08x' % (size, addr), 100)
        ...
        try:
            ...
            while bytesWritten < data.getSize():
                ...
                for i in range(0, len(argLists), MAX_REPEATS):
                    ...
                    progress.progress('Wrote %d bytes' % bytesWritten, bytesWritten)
